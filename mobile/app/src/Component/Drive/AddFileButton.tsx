...
import { View, Text, Button, TouchableOpacity, ToastAndroid, ProgressBarAndroid, Image } from 'react-native';
  ...
  return (
    <View>
      ...
      {uploadingFiles.length > 0 && (
        <View style={{ position: 'absolute', bottom: 16, right: 16, maxWidth: 250 }}>
          {uploadingFiles.map((file) => (
            <View key={file.id} style={{ marginBottom: 10 }}>
              <TouchableOpacity
                onPress={() => {
                  setUploadingFiles((prevUploadingFiles) =>
              ...
              <ProgressBarAndroid
                style={{ height: 20 }}
                styleAttr={file.error ? 'Horizontal' : 'Horizontal' /* animated progress bar */}
                indeterminate={file.error}
                progress={file.error ? 1 : file.progress}
              />
            </View>
          ))}
        </View>
